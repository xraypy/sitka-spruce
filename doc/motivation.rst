...
Many of these tools are useful and very good, but we felt that there
was still room for one more viewer.  The main features that (we hope)
differentiate Sitka Spruce from existing tools include:
  ...
  * by using `wxPython`_, we plan to use Sitka as a reader for tools
    ...
    XAS data.
  * making it easy to extract data from the complex data files for
